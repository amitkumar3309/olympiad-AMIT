...
**`tests/emailDelivery.test.ts` — the Milestone 25 suite**, covering why a verification email used to take a long time to arrive. It is deliberately a separate file from `notifications.test.ts`: that one is about *what* gets sent and to whom, this one is about *when* and by which driver.

Three groups, and the first two test things that cannot be seen from a passing happy path:

- **`keepAlive()`** — a fake `Symbol.for('@vercel/request-context')` is installed on `globalThis` so both branches are exercised without a real platform. It asserts that work is handed to `waitUntil` when a context exists (the fix), that a missing context returns `detached` rather than throwing (local development, and a platform that stops publishing the global), and that a rejected background job never becomes an unhandled rejection — which would take the process down over one undeliverable email.
- **The sweep** — that `next()` is called **synchronously**, so a housekeeping query can never sit in front of a page load, and that `shouldSweep()` is `false` under test. That second one is asserted rather than left implicit: delivery is awaited inline under test for determinism, and a sweep firing on its own schedule beside it would make every assertion about `attempts` flaky.
- **Timing and abandonment** — that a delivered row records both halves of its latency, that an undelivered one leaves them **null rather than zero** (the same rule `StatTile` follows), that both are published by the delivery console, and that a row whose attempts were spent without ever reporting an outcome is marked `failed` with a stated reason instead of sitting `pending` for ever. That last state is written directly, because the only way to produce it naturally is to kill the process mid-send.

Two things about `notifications.test.ts` are worth knowing when adding tests here:
