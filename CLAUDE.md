...
/backend
  ...
  src/services/               question + taxonomy business rules (Milestone 4);
                              ...
                              and grading.ts — THE marking rules, shared by all;
  ...
  src/routes/v1/              auth, me, analytics, practice, mockTests,
                              ...
                              dailyChallengesAdmin, questions (student reads),
...
- **XP and achievements are reached only through their services.** Never insert a `StudentActivity` row directly — call `recordActivity()`, which owns both what an event is worth (`lib/xp.ts`) and its once-per-day/once-per-account rule. Never query the database from `lib/achievements.ts` — add a field to `ProgressFacts` and supply it from the service that owns that data (as `getChallengeFacts()` does). Both seams exist so "what is this worth?" and "what earns this badge?" each have exactly one answer.
...
- The **backend** has a test suite: `vitest` + `supertest`, plus `mongodb-memory-server` for integration tests against a **real** MongoDB — **455 tests across 15 files**. Run with `npm test --prefix backend` (from inside `backend/` when offline; see [`TESTING.md`](TESTING.md)). The **frontend** still has none. See [`TESTING.md`](TESTING.md).
