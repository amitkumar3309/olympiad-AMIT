...
- **The answer key must never reach the client before it should be revealed.** There is exactly one function that reveals a correct answer — `sessionReviewView()` in `services/practiceService.ts` — and it throws unless the session is `submitted`. The in-progress view composes the shared answer-stripped `services/questionView.ts`. If you add a surface that serves questions, reuse `studentQuestionView`; do not hand-write a projection, because a forgotten field is an answer leak. Grading is server-side, always: the browser is never given the means to mark its own work.
- **A practice session snapshots the answer key at draw time.** Grading reads that snapshot, not the live `Question`, so editing a question cannot retroactively change a paper somebody has already sat. Do not "simplify" grading to re-read the question.
