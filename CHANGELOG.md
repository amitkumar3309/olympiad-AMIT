...
"A.M.I.T. OLYMPIAD" text beside it should go. The size was fixed; the text stays, and the
[`DECISIONS.md`](DECISIONS.md) ADR records why.

**The box was not the problem.** `assets/logo.png` is a stacked lockup — measured, the
emblem is `y 56–682` of 1254 and the AMIT wordmark, the expansion and the tagline are the
rest — so a 32px box was showing a ~16px emblem surrounded by four bands of unresolvable
type. Enlarging it would have scaled the mush up too.

- New `assets/logo-mark.png`: the emblem alone, 256×256, 55 kB. Generated by
  `frontend/scripts/crop-logo-mark.cjs`, which is dependency-free (node's own `zlib`, a
  hand-rolled PNG reader/writer) so regenerating it adds no build-time image library.
  `.cjs` because `frontend/package.json` is `"type": "module"`, where `require` is a
  syntax error.
- **It is a circle** (owner's follow-up, same day), at **44px**, **48px from 1024px**.
  That is why the generator *pads* rather than crops: `border-radius` at 50% slices
  anything outside the inscribed circle, and the emblem is wider than its own ring —
  measured, its minimal enclosing circle is radius **371 about (622, 372)** while the
  ring's equator at y=382–386 spans x 294–958, because the star clears the ring at the
  top right and the book clears it at the bottom. A tight crop would cut both. The
  emblem is composited onto a white square sized from that measured radius plus 8%, and
  the script prints how close the outermost ink lands to the edge — **93.8% of the
  radius**, so nothing clips. `logo.png` is untouched and is still the certificate's.
- **The first paint got 975 kB lighter.** `logo.png` sat in the eager bundle because
  `Navbar` is imported by the eager `Landing`; it is now only in the lazy `Certificate`
  chunk. Verified against the emitted chunks, not assumed.
- New `--logo-plate` token, white in **both** themes and declared in both blocks like
  `--card-border`. The artwork is opaque RGB drawn in navy and orange, which disappears on
  the dark theme's black page and royal-blue pill; a light plate is how a logo drawn for
  paper survives a dark theme.
- The image is now `alt="" aria-hidden`. It had been carrying the full expansion on its
  `alt` *and* sitting beside the wordmark text, so the one link announced the name twice.
  The expansion is still on the link's `title` and in the page metadata.

Verified in a browser at 375 and 1280px in both themes, with transitions killed before
measuring: 44/48px as specified, `border-radius` resolving to 999px, white disc over the
`rgba(18, 30, 71, 0.82)` dark pill, zero horizontal overflow at 375px, the wordmark not
ellipsised, and the mark inspected at 180px to confirm the star and the book are inside the
circle. `tsc -b`, `oxlint` and `vite build` all pass.
