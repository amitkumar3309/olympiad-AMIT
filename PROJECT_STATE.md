...
- **Not in the navbar**, deliberately. It is on the link's `title`, and in the page `<title>` and meta description, so assistive technology and search engines still reach it. It is no longer on the logo's `alt`: the navbar image is now the emblem alone (`assets/logo-mark.png`) with the four-letter wordmark as real text beside it, so an `alt` would make the one link announce the name twice.
