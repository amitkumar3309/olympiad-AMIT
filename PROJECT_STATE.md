...
_Last updated: 2026-08-12 (Milestone 7 — Mock Test System, implemented and verified end-to-end)._
...
**Milestone 7 — Mock Test System.** Preceded by Milestone 6 (Practice Zone), Milestone 5 (student profile and dashboard), Milestone 4 (complete question bank), Milestone 3 (RBAC and user management), Milestone 2 (complete authentication), Milestone 1 (backend & database foundation) and Phase 0 (repository audit).

## Current Milestone

None in progress. Milestone 7 is complete; awaiting owner selection of Milestone 8 (see "Immediate Next Task").
...
models/                   15 models, one file each (+ MockTest, MockTestAttempt)
...
- **Three attempt-shaped collections now exist** (`PracticeSession`, `MockTestAttempt`, and the unwritten `ExamAttempt`). The dangerous duplication was extracted — one served-question shape, one grader — but the surrounding documents still resemble each other, and a fourth would be a sign the separation needs revisiting rather than repeating.
...
**Owner decision: Milestone 8.** The strongest candidate is now **the official exam: `ExamAttempt` → `Result` → certificates**, and it is a shorter job than it was a day ago.
