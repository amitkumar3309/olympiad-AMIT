...
**`INVOICE_ORG_EMAIL` and `INVOICE_ORG_PHONE` have a second job since Milestone 25 Phase C**: they are also the support contact printed in the footer of **every** email, via `config.support`. That is deliberate rather than accidental reuse — they are one fact ("how to reach this organisation"), named after the surface that needed them first, and a second pair of variables holding the same address would be a second pair to keep in step. Both still default to what the platform publishes in its own footer, so this is correct with no configuration. If you set them for invoicing, check the wording suits a verification email too.

**Delivery speed is not configurable, deliberately (Milestone 25 audit).** These six variables are the *entire* email configuration — there is no timeout, retry, pool-size, batch-size or sweep-interval variable, and none was added when delivery was made fast. Two reasons. The SMTP timeouts (8 s connect, 8 s greeting, 15 s socket, in `lib/email.ts`) must stay under the outbox's 60-second visibility timeout or a slow send can be overtaken by a second drain and mail two verification links; that is arithmetic keeping the queue correct, not a deployment preference, and an environment variable would invite somebody to break it. And the retry backoff, batch size and sweep throttle are properties of how the queue recovers, which no deployment has a different correct answer for. If you find yourself wanting to raise a timeout, the thing to read is `/admin/email-deliveries` — `providerMs` there tells you whether the provider is actually slow.
