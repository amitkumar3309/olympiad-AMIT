...
- **The Milestone 25 timing instrumentation carries durations and nothing else.** The `registration.timing` log line is four millisecond figures plus the `studentId` — the human-facing competitor number that is already on every other log line in that handler. No address, no token, no password, no hash, no key. The `providerMs` / `queuedForMs` fields stored on `EmailOutbox` and published by `/admin/email-deliveries` are likewise numbers. This was a constraint on the change, not a review finding: an observability feature is exactly the kind of thing that acquires "just log the payload so we can debug it", and the message body is deliberately still never returned by the delivery console.
- **Every value interpolated into an email's HTML part is escaped** (Milestone 25 Phase C). `buildNotificationEmail` puts a **staff-authored** title and body straight into the markup, so before this an administrator could have placed arbitrary markup — including a link of their own choosing — into a message arriving under this platform's name and addressed to a child. It is a low-privilege-to-reader escalation rather than a classic XSS (mail clients strip script), and it was also a live rendering bug: "everyone scoring &lt; 50 should revise" lost the rest of its sentence to an unclosed tag. Escaping is applied to **every** interpolation including URLs we generated ourselves, because "this value is safe today" is what makes the next caller unsafe.
